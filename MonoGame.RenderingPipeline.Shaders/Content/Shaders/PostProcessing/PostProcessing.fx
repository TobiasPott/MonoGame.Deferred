...
float4 VignetteChromaShiftPixelShaderFunction(float4 pos : SV_POSITION, float2 texCoord : TEXCOORD0) : SV_TARGET0
{
	...
	//Load base color
    float3 base = ScreenTexture.Sample(ScreenTextureSampler, pos.xy / Resolution).rgb;
	...
	if (dist > 0.1)
	{
        ...
        float chromaR = ScreenTexture.Sample(ScreenTextureSampler, texCoord.xy + chromaDist).r;
